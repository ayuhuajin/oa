<template>
  <div id="#sendDetail">
    <mt-header fixed title="约稿">
      <router-link to="/manuscripts-list" slot="left">
    ...
    <div class="content-wrap">
      ...
      <div class="inputPicker">
        <span>发起时间:</span>
        <input type="text" readonly="" placeholder="请输入发起时间" @click='openPicker(2)' v-model='list.CreateTime'>
      </div>
      <div class="inputPicker">
        <span>截止时间:</span>
        <input type="text" readonly="" placeholder="请输入截止时间" @click='openPicker(1)' v-model='list.ClosingDate'>
      </div>
      <div class="download">
        <p>附件下载:</p>
...
import {ajaxManuscriptsAdd} from '../../api/api.js'
...
export default {
  ...
  methods: {
    init: function () {
      ...
      _this.list.CreateTime = _this.pickerVisible
    },
    searchAdd () {
      let _this = this
      if (_this.list.NotifyObject.length !== 0) {
        ajaxManuscriptsAdd(_this.list, function (data) {
          _this.$router.push({path: '/manuscripts-list'})
        })
      } else {
        this.show = true
      }
