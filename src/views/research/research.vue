<template>
  <div id="#sendDetail">
    <mt-header fixed title="调研课题">
      <router-link to="/research-list" slot="left">
    ...
    <div class="content-wrap">
      ...
      <div class="inputPicker">
        <span>反馈截止时间:</span>
        <input type="text" readonly="" placeholder="请输入反馈截止时间" @click='openPicker(1)' v-model='list.ClosingDate'>
      </div>
      <div class="inputPicker">
        <span>课题报告提交截止时间:</span>
        <input type="text" readonly="" placeholder="请输入课题报告提交截止时间" @click='openPicker(2)' v-model='list.SubmissionDeadline'>
      </div>
      <div class="download">
        <p>附件下载:</p>
...
import {ajaxResearchAdd} from '../../api/api.js'
...
export default {
  ...
  methods: {
    init: function () {
      ...
      _this.list.SubmissionDeadline = _this.pickerVisible
    },
    searchAdd () {
      let _this = this
      if (_this.list.NotifyObject.length !== 0) {
        ajaxResearchAdd(_this.list, function (data) {
